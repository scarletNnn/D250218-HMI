...
public class DataAccess : IDataAccess
{
    ...
    public async Task<List<AlertMessage>> GetAlertMessage(DateTime date)
    {
        ...
        {
            return await ctx.AlertMessages.Where(a => a.Timestamp.Day == date.Day).ToListAsync();
        }
    }
    ...
    /// 添加生产信息
    /// </summary>
    /// <param name="productionMessage"></param>
    /// <returns></returns>
    public async Task InsertProductionMessage(ProductionMessage productionMessage)
    {
        using (MyDbContext ctx = new MyDbContext())
        {
            await ctx.ProductionMessages.AddAsync(productionMessage);
            await ctx.SaveChangesAsync();
        }
    }
    ...
    /// <param name="date"></param>
    /// <returns></returns>
    public async Task<int> GetProductionMessage(DateTime date, int hours)
    {
        using (MyDbContext ctx = new MyDbContext())
        {
            return await ctx.ProductionMessages.Where(
                a => a.Timestamp.Day == date.Day && a.Timestamp.Hour == hours && a.Value == true
            )
                .CountAsync();
        }
    }
